...
T = TypeVar("T", bound="RotorIn")


@_attrs_define
class RotorIn:
    """ONE rotor for a steady or sweep run.
    ...
    coaxial trio.
        ...
        Attributes:
            ...
            esc_timing (RotorInEscTiming | Unset): Six-step commutation advance preset (low 7.5°/medium 15°/high 22.5°/auto
                duty-scheduled); FOC ignores it Default: RotorInEscTiming.MEDIUM.
            esc_type (RotorInEscType | Unset): ESC commutation type. Selects K_volt, copper_mult, iron_mult, the six-step
                commutation-advance physics and the throttle->duty map — not a cosmetic flag. Single-sourced (ESC-7):
                app/constants_esc.py. Default: RotorInEscType.SIX_STEP.
            motor_component_id (None | str | Unset):
            motor_cooling_source (RotorInMotorCoolingSource | Unset): Per-rotor-group motor cooling mode. cowling = still
                air; prop_exit_velocity = forced convection from the prop slipstream (default); custom = fixed velocity or
                direct thermal resistance. Default: RotorInMotorCoolingSource.PROP_EXIT_VELOCITY.
            ...
            motor_t_w (float | None | Unset): Optional fixed winding temperature (°C, -60…250); omit to let the model
                compute it.
            rotation_sense (RotorInRotationSense | Unset): Direction of rotation about the downstream axis: 1 or -1.
    ...
    esc_timing: RotorInEscTiming | Unset = RotorInEscTiming.MEDIUM
    esc_type: RotorInEscType | Unset = RotorInEscType.SIX_STEP
    motor_component_id: None | str | Unset = UNSET
    motor_cooling_source: RotorInMotorCoolingSource | Unset = RotorInMotorCoolingSource.PROP_EXIT_VELOCITY
    ...
    rotation_sense: RotorInRotationSense | Unset = RotorInRotationSense.VALUE_1
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        esc_timing: RotorInEscTiming | Unset
        if isinstance(_esc_timing, Unset):
            esc_timing = UNSET
        else:
            esc_timing = RotorInEscTiming(_esc_timing)

        _esc_type = d.pop("esc_type", UNSET)
        esc_type: RotorInEscType | Unset
        if isinstance(_esc_type, Unset):
            esc_type = UNSET
        else:
            esc_type = RotorInEscType(_esc_type)
        ...
        motor_cooling_source: RotorInMotorCoolingSource | Unset
        if isinstance(_motor_cooling_source, Unset):
            motor_cooling_source = UNSET
        else:
            motor_cooling_source = RotorInMotorCoolingSource(_motor_cooling_source)
        ...
        rotation_sense: RotorInRotationSense | Unset
        if isinstance(_rotation_sense, Unset):
            rotation_sense = UNSET
        else:
            rotation_sense = RotorInRotationSense(_rotation_sense)
